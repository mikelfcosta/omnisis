...
export async function populateCards(smartCardsToCreate: number = 10) {
  ...
  const inactiveCard = new omniSmartCards({ _id: faker.internet.ipv6(), assigned: true, student: faker.internet.userName(), active: false });
  const unassignedCard = new omniSmartCards({ _id: faker.internet.ipv6(), assigned: false });
  try {
